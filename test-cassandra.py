...
for cluster_count in (1, 2, 4, 8):
    cluster = Cluster(cluster_ips[:cluster_count])
    ...
    for i in range(1000):
        s.execute(
            "INSERT INTO kvpair (k, v) VALUES (%s, %s)",
    ...
    print("insert %d items time: %sus" % (test_size, insert_time.microseconds))
    ...
    print("lookup %d items time: %sus" % (test_size, lookup_time.microseconds))
    ...
    # total time
    total_time = (
        insert_time.microseconds + lookup_time.microseconds + delete_time.microseconds
